...
class ScenarioSynthesisSerializer(serializers.ModelSerializer):
    ...
    fund_id = serializers.IntegerField(source='fund.pk', read_only=True)
    created_at = serializers.DateTimeField(format="%Y-%m-%dT%H:%M:%S%z", read_only=True)
    updated_at = serializers.DateTimeField(format="%Y-%m-%dT%H:%M:%S%z", read_only=True, allow_null=True)
    ...
    class Meta:
        ...
        fields = [
            'synthesis_id', 'fund_id', 'synthesis_name',
            'description', 'is_deleted', 'created_at',
            'created_by', 'updated_at', 'scenarios', 'scenario_ids'
        ]
        read_only_fields = ['synthesis_id', 'fund_id', 'created_at', 'updated_at', 'created_by']

    def get_scenarios(self, obj):
        scenarios = ScenarioList.objects.filter(
            scenario_id__in=obj.scenario_mappings.values_list('scenario_id', flat=True)
        )
