...
function normalizeDealDetail(payload) {
  ...
  return {
    ...
    pipelineEntryDate: row?.pipeline_entry_date ?? row?.deal_date ?? row?.created_at ?? "",
    latestUpdateAt: row?.updated_at ?? row?.updatedAt ?? row?.created_at ?? null,
    latestUpdateByName:
      row?.updated_by?.name ??
      row?.updated_by_name ??
      row?.last_updated_by?.name ??
    ...
    esgRisk: row?.esg_risk?.id ?? row?.esg_risk_id ?? null,
    esgNotes: row?.esg_notes ?? "",
    additionalNotes: row?.additional_notes ?? "",
    emergingMarketThesis: row?.emerging_market_thesis ?? "",
    exitType: row?.exit_type?.id ?? row?.exit_type_id ?? null,
    exitRelevantInfo: row?.exit_relevant_information ?? "",
    dealType: row?.deal_type?.id ?? row?.deal_type_id ?? null,
...
export function mapInfoFormToPayload(form, { countries = [], currencies = [] } = {}) {
  ...
  return {
    ...
    status_reason: normalizeText(form.statusReason),
    ...
    esg_risk_id: normalizeNullable(form.esgRisk),
    esg_notes: normalizeText(form.esgNotes),
    additional_notes: normalizeText(form.additionalNotes),
    emerging_market_thesis: normalizeText(form.emergingMarketThesis),
    exit_type_id: null,
    exit_relevant_information: "",
    deal_type_id: normalizeNullable(form.dealType),
    website: "",
    registration_number: "",
    address: "",
    zip_code: "",
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
      const [
        ...
        api.get(`${DEALFLOW_TAXONOMY_ENDPOINT}?type=operation_type`),
        api.get(`${DEALFLOW_TAXONOMY_ENDPOINT}?type=co_investor_type`),
        api.get(`${DEALFLOW_TAXONOMY_ENDPOINT}?type=exit_route`),
        api.get(`${DEALFLOW_TAXONOMY_ENDPOINT}?type=exit_counterparty`),
        api.get(`${DEALFLOW_TAXONOMY_ENDPOINT}?type=exit_horizon`),
        api.get(`${DEALFLOW_TAXONOMY_ENDPOINT}?type=esg_risk`),
      ...
      setOptions({
        ...
        operationTypes: formatDropdownOptions(operationTypes),
        coInvestorTypes: formatDropdownOptions(coInvestorTypes),
        exitRoutes: formatDropdownOptions(exitRoutes),
        exitCounterparties: formatDropdownOptions(exitCounterparties),
        exitHorizons: formatDropdownOptions(exitHorizons),
        esgRisks: formatDropdownOptions(esgRisks),
