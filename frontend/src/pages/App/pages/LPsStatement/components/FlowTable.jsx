...
const OPERATIONS_ROWS = [
  // ... your full operations data EXACTLY as you had it ...
  {
    id: 1,
...
/* ------- LPs rows and Share class rows exactly as I sent earlier ------- */
...
export default function FlowTable({ operationFilter, search, breakdown }) {
  ...
  const { rows: baseRows, columns: columnsByView } =
    BREAKDOWN_CONFIG[breakdown];
    ...
    if (search && search.trim()) {
      ...
      data = data.filter((r) => {
        if (breakdown === "operations") {
          return (r.label || "").toLowerCase().includes(s);
        }
        if (breakdown === "lps") {
          return (r.lp || "").toLowerCase().includes(s);
        }
            ...
            <tr>
              {columns.map((col) => (
                ...
                >
                  {col.header}
                  {col.sortable && (
                    <span className="cf-sort">{sortIcon(col.key)}</span>
                  )}
                </th>
              ))}
            </tr>
