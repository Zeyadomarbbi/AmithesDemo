...
const AddPeriodModal = ({ open, onClose, onSave }) => {
  ...
  const applyCalendar = () => {
    ...
    setEndDate(formatDisplay(selectedDate));
    setShowCalendar(false);
  };

  const endText = endDate || "00/00/00";
  const isEndPlaceholder = !endDate;
          ...
          <form className="period-modal-body" onSubmit={handleSubmit}>
            ...
            <div className="period-field period-field-end">
              ...
              <div className="period-input-with-icon">
                <button
                  type="button"
                  className="period-input period-input-button"
                  onClick={() => setShowCalendar(true)}
                >
                  <span
                    className={
                      "period-input-text" +
                      (isEndPlaceholder ? " period-input-placeholder-text" : "")
                    }
                  >
                    {endText}
                  </span>
  ...
  >
    ...
    />
        ...
        {/* ===== PAGE 2: CALENDAR ===== */}
        {showCalendar && (
          <div className="period-modal-body period-calendar-body">
            {/* Selected date pill */}
            <div className="period-calendar-selected-row">
              <button className="period-calendar-pill" type="button">
                {selectedDate
            ...
            <div className="period-calendar-header">
              <button
                type="button"
                className="period-calendar-nav"
                onClick={goPrevMonth}
              >
                ‹
              </button>
              <span className="period-calendar-month-label">
              ...
              {cells.map((day, idx) => {
                ...
                return (
                  <button
                    ...
                    className={
                      "period-calendar-day" +
                      (isSelected ? " period-calendar-day-selected" : "")
                    }
                    disabled={!day}
